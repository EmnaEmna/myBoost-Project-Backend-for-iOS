...
async function donedash (groupId){
    const projects = await Project.find({ group:groupId })
    console.log("groupdId"+groupId)
  //findbyid
    const allproj = projects.length;
    console.log("this groupe "+groupId+" have "+allproj+"  projects");
    const doneprojtslist = await Project.find({group: groupId, status: 'done'});
    const doneprojtslenght = doneprojtslist.length;
    console.log("state X ones : " + doneprojtslenght);
    const doneprojts = (doneprojtslenght/allproj).toFixed(2) ;
    const data = 
    
    {"allproj":allproj,
    "doneprojtslenght":doneprojtslenght,
    "doneprojts":doneprojts}
      ;     
    return data ;
  }

  const getgroupwithdata = asyncHandler(async (req, res) => {
    const groups = await Group.find({ emails:req.body.email }) ;
    const datas = [];
  
    await Promise.all(groups.map(async (element) => {
      const data = await donedash(element._id);
      const elementjson = element.toJSON();
      const mergedObj = Object.assign({}, elementjson, data);
      datas.push(mergedObj);
    }));
  
    res.status(200).json(datas);
  });

  const getallgroupwithdata = asyncHandler(async (req, res) => {
